/**
 * Script to switch the Orca Liquidity Agent to Solana Mainnet
 * This updates the .env file with the correct mainnet configuration
 */
...
const MAINNET_CONFIG = {
  ...
  // Real mainnet USDC/SOL whirlpool address
...
async function main() {
  try {
    logger.info('Switching Orca Liquidity Agent to Solana Mainnet...');
    ...
    Object.entries(MAINNET_CONFIG).forEach(([key, value]) => {
    ...
    logger.info('Successfully switched to Solana Mainnet!');
    logger.info('Please restart the application for changes to take effect.');
    
  } catch (error) {
    logger.error('Error switching to mainnet:', error);
  }
}
